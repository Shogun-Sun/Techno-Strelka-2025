...
export class GoogleCloudService {
  ...
  async accessGeminiApi(prompt: string): Promise<string> {
    const context = `
      ...
      Хоть ты и оператор связи Т2, но ты не можешь работать с номерами телефона, смотреть тариф и т.д, поэтому говори, что через какое-то время 
      к чату подключиться оператор, который сможет помочь с этим. Не справшивай у пользователя телефон. 
    `;
